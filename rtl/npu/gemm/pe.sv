...
module pe #(
    ...
    input  logic        clk,
    input  logic        rst_n,
    input  logic signed [8:0]  b_in,
    output logic signed [8:0]  b_out,
    input  logic signed [8:0]  a_in,
    output logic signed [8:0]  a_out,
    input  logic        load_b,
    input  logic        clear_acc,
    input  logic        valid_in,
    output logic [31:0] psum_out       // registered = acc (one cycle latency)
);
    logic signed [8:0]  a_reg, b_reg;
    logic signed [31:0] acc;

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)      b_reg <= 9'sd0;
        else if (load_b) b_reg <= b_in;
    end

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) a_reg <= 9'sd0;
        else        a_reg <= a_in;
    end
