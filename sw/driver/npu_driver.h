...
// 2D DMA store: copy rows of row_bytes from NPU SRAM to external memory.
int npu_dma_2d_st(npu_dev_t *d, uint32_t ext_addr, uint32_t sram_off,
                  uint32_t rows, uint32_t row_bytes,
                  uint32_t ext_stride, uint32_t sram_stride);

// Load and execute a serialized .npu program generated by tools/codegen.
// Current RTL IF/ID has a 4-word local instruction memory, so programs are
// limited to at most four instructions.  Returns 0 on success.
