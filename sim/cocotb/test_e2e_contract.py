...
async def test_e2e_contract(dut):
        ...
        if val != 0 and val != last:
            ...
            if ch == "F":
                raise AssertionError(
                    ...
                    f"results={[hex(_dram_word(dut, RESULT_BASE + i * 4)) for i in range(5)]}"
                )
        last = val
    else:
        try:
            dut._log.info(
                ...
                f"dma=0x{_dram_word(dut, RESULT_BASE + 8):08X} "
                f"valu=0x{_dram_word(dut, RESULT_BASE + 12):08X} "
                f"sfu=0x{_dram_word(dut, RESULT_BASE + 16):08X} "
    ...
    dma_ret = _dram_word(dut, RESULT_BASE + 8)
    valu_ret = _dram_word(dut, RESULT_BASE + 12)
    sfu_ret = _dram_word(dut, RESULT_BASE + 16)
    ...
    assert dma_ret == 0, f"DMA contract returned 0x{dma_ret:08X}"
    assert valu_ret == 0, f"VALU contract returned 0x{valu_ret:08X}"
    assert sfu_ret == 0, f"SFU contract returned 0x{sfu_ret:08X}"
    assert "".join(progress[-4:]) == "dvsP", f"unexpected progress {progress}"
